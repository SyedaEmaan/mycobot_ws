...
BringupResult MyCobotCSV::init_soem()
{
    ...
    if (grp->Obytes != expected_obytes) {
        RCLCPP_ERROR(logger_,
            "Output PDO size mismatch: mapped %d bytes, expected %zu. "
            "Did slave_csv_config() succeed on every slave?",
            grp->Obytes, expected_obytes);
        ecx_close(&context_);
        return BringupResult::PDO_SIZE_MISMATCH;
    }
    if (grp->Ibytes != expected_ibytes) {
        RCLCPP_ERROR(logger_,
            "Input PDO size mismatch: mapped %d bytes, expected %zu. "
            "Did slave_csv_config() succeed on every slave?",
            grp->Ibytes, expected_ibytes);
        ecx_close(&context_);
        return BringupResult::PDO_SIZE_MISMATCH;
    }
