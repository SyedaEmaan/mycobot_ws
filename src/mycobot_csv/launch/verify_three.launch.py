"""
verify_three.launch.py — three-joint CSV verification.

Same structure as verify_two.launch.py but uses test_three_joints.urdf and
controllers_three.yaml. forward_velocity_controller takes a Float64MultiArray
of three velocities (one per joint).
"""
import os
...
def generate_launch_description():
    ...
    urdf_file = os.path.join(pkg_share, "urdf", "test_three_joints.urdf")
    with open(urdf_file, "r") as f:
        robot_description_content = f.read()

    controllers_yaml = os.path.join(pkg_share, "config", "controllers_three.yaml")
