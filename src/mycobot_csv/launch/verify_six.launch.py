"""
verify_six.launch.py — six-joint CSV verification.

Same structure as verify_five.launch.py but uses test_six_joints.urdf and
controllers_six.yaml. forward_velocity_controller takes a Float64MultiArray
of six velocities (one per joint).
"""
import os
...
def generate_launch_description():
    ...
    urdf_file = os.path.join(pkg_share, "urdf", "test_six_joints.urdf")
    with open(urdf_file, "r") as f:
        robot_description_content = f.read()

    controllers_yaml = os.path.join(pkg_share, "config", "controllers_six.yaml")
