"""
csv_moveit.py — MoveIt + real EtherCAT hardware on all 6 joints via mycobot_csv.

Brings up:
  - ros2_control_node hosting the mycobot_csv hardware system
...
def generate_launch_description():
    pkg_moveit = get_package_share_directory('kerabot_moveit_config')

    urdf_path = os.path.join(pkg_moveit, 'config', 'my_robot_arm.urdf.xacro')
    ...
    moveit_config = (
        MoveItConfigsBuilder("my_robot_arm", package_name="kerabot_moveit_config")
        .robot_description(file_path=urdf_path)
        .to_moveit_configs()
    )

    ompl_planning_yaml = load_yaml("kerabot_moveit_config", "config/ompl_planning.yaml")
    controllers_yaml = os.path.join(pkg_moveit, 'config', 'ros2_controllers.yaml')

    # 1. ros2_control_node — owns the hardware (mycobot_csv)
